...
class PaymentRepository {
  ...
  Future<bool> bookingsApi(CartData cartData, String payment) async {
    try {
      ...
      BookingDataModel bookingDataModel = BookingDataModel(
          couponId: "${couponData.id == 0 ? "" : couponData.id}",
          userId: user.id,
      ...
      ShoppingRepository shoppingRepository = ShoppingRepository(
          auth: auth, dioClient: dioClient, sharedPreferences: sharedPreferences);
      ...
      // log(jsonData);
      await dioClient.makePayment(jsonData);
    } catch (e) {
      print(e);
      return false;
    }
